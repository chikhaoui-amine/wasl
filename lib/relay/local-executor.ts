...
export class LocalMcpExecutor {
  ...
  private async dispatchTool(name: string, args: Record<string, unknown>): Promise<unknown> {
    switch (name) {
        ...
        await this.mutateStore("lifeos-notes", (current) => {
          const notes = (current.notes ?? []).map((n) => {
            ...
            const mod = {
              ...
              section: typeof args.section === "string" ? (args.section.trim() || undefined) : n.section,
              updatedAt: Date.now(),
            };
            updatedNote = mod;
            return mod;
      ...
      case "note_categories_list": {
        const doc = await this.adapter.getStore("lifeos-notes");
        const categories = (doc?.state.categories ?? [])
          .sort((a, b) => a.name.localeCompare(b.name))
          .map(({ id, name, color, icon, sections }) => ({ id, name, color, icon: icon ?? null, sections: sections ?? [] }));
        return page(categories, args);
      }

      case "note_categories_get": {
        const doc = await this.adapter.getStore("lifeos-notes");
        const category = exactById(doc?.state.categories ?? [], args.id, "Note category");
        return { item: { ...category, icon: category.icon ?? null, sections: category.sections ?? [] } };
      }
      ...
      case "add_note_category": {
        const category = {
          ...
          color: typeof args.color === "string" ? args.color : "var(--accent)",
          icon: typeof args.icon === "string" ? args.icon : undefined,
          sections: Array.isArray(args.sections) ? args.sections.map(String).filter(Boolean) : undefined,
        };
        await this.mutateStore("lifeos-notes", (current) => ({
          ...current,
